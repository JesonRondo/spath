      ...
      f.src.forEach(function(filename) {
        if (grunt.file.exists(filename)) {
          if (!grunt.file.isDir(filename)) {
            ...
            var splice_filename = key.split('.');
            if (splice_filename[splice_filename.length - 2]) {
              splice_filename[splice_filename.length - 2] += '_' + hash;
            }
            splice_filename = splice_filename.join('.');

            // out file
            
            var outPath = path.resolve(path.dirname(filename), basedir, splice_filename);
            fs.renameSync(filename, outPath);

            hashes[key] = splice_filename;
          }
        } else {
